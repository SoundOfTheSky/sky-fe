...
const { t } = basicStore
export default function StartReviewSession(properties: { lessons?: boolean }) {
  // === Hooks ===
  const { lessons, reviews, settings, offlineUnavailable, ready } = useStudy()!
  ...
  const subjects = createMemo(() =>
    properties.lessons ? lessons() : reviews(),
  )
  const settingsScoped = createMemo(() =>
    properties.lessons ? settings().lessons : settings().reviews,
  )
      ...
      >
        <div class={s.settingsWrapper}>
          <div title={t('STUDY.SETTINGS.AMOUNT_DESC')}>
            <div>
              {t('STUDY.SETTINGS.AMOUNT')}:{settingsScoped().amount}
            </div>
            <input
              type='range'
              min='1'
          ...
          <div title={t('STUDY.SETTINGS.BATCH_DESC')}>
            <div>
              {t('STUDY.SETTINGS.BATCH')}: {settingsScoped().batch}
            </div>
            <input
              type='range'
              min='1'
