...
function getProvided() {
  ...
  const offlineUnavailable = createMemo(
    () =>
      !basicStore.online() &&
      (syncStore.status() === SYNC_STATUS.ERRORED ||
        syncStore.status() === SYNC_STATUS.IDLE),
  )
  ...
  const startDate = createMemo(() => {
    ...
    // 44 weeks back
    return new Date(
      $today.getTime() - $today.getDay() * DAY_MS - 26_611_200_000,
    )
  })
  ...
  async function updateStats() {
    ...
    const data = Array.from({
      length: Math.ceil((startTime + 31_449_600 - startTime) / 86_400),
    }).fill(0) as number[]
    for await (const { value } of database.transaction(
      'studyAnswers',
      'readwrite',
    ).store) {
